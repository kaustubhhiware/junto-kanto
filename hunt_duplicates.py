...
def getNodes(filename):
    """
        return a dict containing all nodes in the file
    """
...
def keepNumof(maximum,restrictLabel):
    """
        ...
        writes surplus nodes from seeds to surplus_nodes.txt in that part
    """
    counter = maximum
    filename= "seeds.txt"
    keep_str = ""
    ...
    for i in range(1,len(data)):
        ...
        # surplus nodes can be added to gold_labels.txt
...
def equalizeNodes():
    """
        ensure #L1 = #L2 in current part seeds only
    """
    L1num = getLabelled("L1")
    L2num = getLabelled("L2")
    print "\n---",L1num,"\t---\t",L2num,"\n"
...
def partRemoveCopy(partnum):
    ...
    U = U1 # universal set is smaller of them
    if len(U2) < len(U1):
        U = U2

    correct_file = "seeds.txt" # from which files nodes must be removed
    if len(U2) < len(U1):
