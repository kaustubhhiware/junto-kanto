		...
		for each in dic[key]:
			...
			for each1 in lis:
				if source.startswith(each1):
					if source[len(each1):] in verbs:
						report_check = 1
			if report_check==1:
				if len(each.split(" "))==2:
					if int(each.split(" ")[1])==1:
						ground.append([source,derived,each1,source[len(each1):]])
						report+=1
						continue
				else:
					ans = 0
	...
	for i in range(len(yes)):
		for j in range(i+1,len(yes)):
			...
			if yes[i][0] in setG and yes[j][0] in setS:
				edgeVSG += 1
			if yes[i][0] in setS and yes[j][0] in setG:
				edgeVSG += 1
...
def main_new():
	count_list = [["PART","L1","L2","Total"]]
	print main(19,"No",[],False,"",1,count_list)
