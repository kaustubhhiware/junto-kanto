...
# This program copies all label_prop_outputs and nodes_dict
# from copy_argument to copy_argument+1
# ex : 1 to 2 , 2 to 3
# author: Kaustubh

def copy(loc):
    """
        Copy all label_prop_outputs and nodes_dict to STEP(loc+1)
    """
    ...
    for i in range(1,31):
        ...
        if i in nope:
            ...
            if os.path.isfile(destfile):
                print "removing older label_prop_output in part",str(i),"\n"
                os.system("rm -f "+destfile)

            copyfile(srcfile,destfile)
            if os.path.isfile(destfile):
                print 'Label_prop_output copied for part',i,'\n'
            else:
                print 'Failure !exit'
                sys.exit()

            # copy nodes_dict as well
            fname = "/nodes_dict.txt"
            srcfile = src + current_part + fname
            destfile = dest + current_part + fname

            if os.path.isfile(destfile):
                print "removing older nodes_dict in part",str(i),"\n"
                os.system("rm -f "+destfile)

            copyfile(srcfile,destfile)
            if os.path.isfile(destfile):
                print 'nodes_dict copied for part',i,'\n'
            else:
                print 'Failure in copy! exit'
                sys.exit()
